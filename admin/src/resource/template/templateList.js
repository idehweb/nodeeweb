...
const PostFilter = (props) => {
  ...
  return (
    <Filter {...props}>
      <SearchInput
        source="Search"
        placeholder={translate('resources.template.search')}
        alwaysOn
      />
      <SearchInput
        source="category"
        placeholder={translate('resources.template.category')}
        alwaysOn
      />
    </Filter>
  );
  ...
  return (
    <List {...props} filters={<PostFilter />} pagination={<PostPagination />}>
      <Datagrid optimized>
        ...
        <FunctionField
          label={translate('resources.template.date')}
          render={(record) => (
            <div className="theDate">
              <div>
                {translate('resources.template.createdAt') +
                  ': ' +
                  `${dateFormat(record.createdAt)}`}
              </div>
              <div>
                {translate('resources.template.updatedAt') +
                  ': ' +
              ...
              {record.views && (
                <div>
                  {translate('resources.template.viewsCount') +
                    ': ' +
                    `${record.views.length}`}
                </div>
              )}
        ...
        />
        <FunctionField
          label={translate('resources.template.actions')}
          render={(record) => (
            <div>
              <div>
                ...
                <a
                  target={'_blank'}
                  href={'/admin/#/builder' + '/template/' + record._id} rel="noreferrer">
                  <NoteAltIcon />
