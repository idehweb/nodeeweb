...
const list = (props) => {
  ...
  if (themeData && themeData.rules && themeData.rules[resource] && themeData.rules[resource].list && themeData.rules[resource].list.header) {
    resourceRules = themeData.rules[resource].list.header;
  }

  console.log("themeData", themeData, resourceRules);
  return (
    ...
    <List {...props} filters={<PostFilter/>} pagination={<PostPagination/>} actions={<ListActions/>}
      ...
      <Datagrid optimized>
        ...
        {resourceRules && (resourceRules).map((r, i) => {
          // let x = Object.keys(resourceRules);
          if (r) {
            if (r.type == "image") {
            ...
            if (r.type == "multiLang") {
              ...
              return <FunctionField label={r.name}
                                    render={record => {
                                      return <><ShowLink base={resource} source={r.name + "." + translate("lan")}
                                        // label={translate("resources."+resource+"."+r.name)}
                                                         theSource={r.name}
                                                         label={r.name}
                                                         sortable={false}/>
                                        <br/>
            ...
            if (r.type == "text") {
              // return JSON.stringify(r.name);
              return <TextField source={r.name}/>;
            }
            if (r.type == "referenceField") {
              // return JSON.stringify(r.name);
              return <ReferenceField
                label={r.name}
                source={r.name}
                reference={r.reference}>
                <TextField source={r.textFieldSource + "." + translate("lan")}/>
              </ReferenceField>;
            }
            if (r.type == "actions") {
              // return JSON.stringify(r.name);
              return <FunctionField label={translate("resources.page.actions")}
                                    render={record => (<div>
                                      {r.pageBuilder && <div>
                                        {/*+"?token="+localStorage.getItem('token')*/}
                                        <a target={"_blank"}
                                           href={"/admin/#/builder" + "/page/" + record._id}><NoteAltIcon/><span
                                          className={"ml-2 mr-2"}>{translate("resources.page.pagebuilder")}</span></a>
                                      </div>}
                                      ...
                                      {r.copy && <div>
                                        <Button
                                          ...
                                          <ContentCopyIcon/><span
                                          className={"ml-2 mr-2"}>{translate("resources.page.copy")}</span>
                                      ...
                                      {r.actions && <div>
                                        <a
                                          ...
                                          <PendingActionsIcon/><span
                                          className={"ml-2 mr-2"}>{translate("resources.page.activities")}</span>
                                        </a>
                                      </div>}
                                      {r.delete && <div>
                                        <DeleteButton/>
