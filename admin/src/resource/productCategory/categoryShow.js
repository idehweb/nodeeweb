...
export const categoryShow = (props) => {
  ...
  const getProducts = async (start, end) => {
    await API.get(`/product/${start}/${end}?_order=DESC`).then(
      ({ data = {} }) => {
        if (data.length !== 0) {
  ...
  return [
    ...
    <Card className={'width1000'} style={{ marginTop: '20px' }}>
      <CardHeader
        title={'  تعداد محصولات  این دسته بندی  ' + products.length + 'محصول'}
      />{' '}
      <span>{}</span>
      <CardContent>
                    ...
                    <TableRow
                      ...
                      <TableCell padding="checkbox">
                        <Checkbox
                          ...
                          onChange={(e) =>
                            e.target.checked && handlerSelectProduct(row)
                          }
                        />
                      </TableCell>
                ...
                return (
                  <div className={'cobm flex-d cobm' + key} key={key}>
                    <div className={'flex-1'}>
                      {comb.options && (
                        <div className={''}>
                          {Object.keys(comb.options).map((item, index) => {
                            return (
                              <div key={index}>
                                {item + ' : ' + comb.options[item] + '\n'}
                              </div>
                            );
                          })}
                        </div>
