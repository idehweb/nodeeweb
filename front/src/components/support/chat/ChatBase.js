    ...
    <React.Fragment>
      {data.type === 'client' ? (
        <div className="yours messages">
          <div className="message last">{data.text}</div>
        </div>
      ) : (
        data.type === 'support' && (
...
const ChatContent = (props) => {
  ...
  const handlerCallBackMessage = async (value) => {
    ...
    if (value) {
      ...
      await axios
        .post('https://idehweb.com/chatgpt/index.php', messageObjClient, {
          headers,
        })
        .then((res) => {
  ...
  return (
    <>
      <section
        ...
        <ChatContent action={true} onClose={() => setToggleChat(false)} />
      </section>
    </>
  );
};
