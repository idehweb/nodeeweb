...
function SiteStatus({ onClick, item, t }) {
  ...
  let [siteStatusMessage, setSiteStatusMessage] = useState(
    store.getState().store.siteStatusMessage || ''
  );
  ...
  useEffect(() => {
    console.log(
      'store.getState().store.siteStatus',
      store.getState().store.siteStatus
    );
  ...
  if (siteStatus) {
    ...
    if (siteStatusMessage) {
      return (
        <div className={'top-bar-alert false'}>
          <div className={'top-bar-alert-inner false'}>{siteStatusMessage}</div>
        </div>
      );
    } else {
      return (
        <div className={'top-bar-alert false'}>
          <div className={'top-bar-alert-inner false'}></div>
        </div>
      );
    }
  }
