...
const theChip = (props) => {
  ...
  return [
    <div className={' mt-5 the-chip row'} key={0}>
      ...
      {showPop && (
        <AlertPopup
          title={t('please choose combination') + ':'}
          canBuy={canBuy}
          show={showPop}
          onHandler={(e) => setShowPop(e)}
          item={{
          ...
          <section
            ...
            <div
              className={'gfd'}
              style={{ marginTop: '20px', float: 'left', color: '#3e3f64' }}>
              <Theprice
                className={
                  ' single-let ' +
          ...
          <section>
            <span
              ...
              تفاوت گارانتی‌های مختلف آروند چیست؟
            </span>
            <p
              style={{
                fontSize: '12px',
              ...
              آروند برروی کالاهای خود علاوه‌بر گارانتی اصالت و سلامت فیزیکی، دو
              گارانتی ۱۸ ماهه سبز آروند و ۱۸ ماهه طلایی آروند همراه با پوشش
              حوادث را ارائه می‌دهد. ضمانت سبز آروند فقط مشکلات خود به خودی
              دستگاه را پوشش می‌دهد. اما ضمانت تکمیلی طلایی، علاوه بر مشکلات خود
              به خودی دستگاه، تمام حوادث وارده توسط فرد را هم تحت پوشش خود قرار
              می‌دهد.
            </p>
          </section>
        </AlertPopup>
...
const ChipInside = ({ opt, actives, onClickChip, showTitle }) => {
  ...
  return (
    <Stack direction="row" className={' mt-2 wrap-stack'} spacing={1}>
      {showTitle && (
        <label className={'the-label-inline'} style={{ width: '100%' }}>
          {opt.name + ':'}
        </label>
      )}
